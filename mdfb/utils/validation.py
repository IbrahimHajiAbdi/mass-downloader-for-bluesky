...
def validate_no_posts(posts: list, account: str, post_types: list, update: bool, did: str, restore: str):
    ...
    if restore:
        if did:
            ...
            raise ValueError(
                f"There are no posts associated with account: {account}, for post_type(s): {post_types}, in the database."
            )
...
def validate_download(args: argparse.Namespace, parser: argparse.ArgumentParser):
    if args.restore:
        if args.did or args.handle:
            parser.error(
                "If using --restore, then cannot use --did, -d or --handle, should pass the handle/did as a value to --restore"
            )
    else:
